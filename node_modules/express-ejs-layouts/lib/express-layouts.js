...
module.exports = function (req, res, next) {
  ...
  res.render = function (view, options, fn) {
    var index, self = this, app = req.app,
      defaultindex = app.get('index');
    ...
    if (options.index === false || ((options.index || defaultindex) === false)) {
      res.__render.call(res, view, options, fn);
      return;
    }

    index = options.index || res.locals.index || defaultindex;
    ...
    res.__render.call(res, view, options, function (err, str) {
      ...
      for (l in options) {
        if (options.hasOwnProperty(l) && l !== 'index' && l !== 'contentFor') {
          locals[l] = options[l];
        }
      }
      ...
      if (options.extractScripts === true || (options.extractScripts === undefined && app.get('index extractScripts') === true)) {
        locals.script = '';
        parseScripts(locals);
      }

      if (options.extractStyles === true || (options.extractStyles === undefined && app.get('index extractStyles') === true)) {
        locals.style = '';
        parseStyles(locals);
      }

      if (options.extractMetas === true || (options.extractMetas === undefined && app.get('index extractMetas') === true)) {
        locals.meta = '';
        parseMetas(locals);
      }

      parseContents(locals);
      res.__render.call(self, index, locals, fn);
    });
  };
  next();
};
